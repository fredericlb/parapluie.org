...
import { SongNodeData, filterSearch } from "../data";
import { useSessionContext } from "../store/session";

const SearchField: React.FC<{
	search: string;
	setSearch: (s: string) => void;
	currentSongAttributes: SongNodeData | null;
	setHoveredNode: (n: string) => void;
}> = ({ search, setSearch, currentSongAttributes, setHoveredNode }) => {
	...
	if (currentSongAttributes) {
		values.forEach((song) => {
			...
			if (
				currentSongAttributes.transitions
					.map((t) => `${t}.mp3`)
					.includes(song.file)
			) {
				transitions.push(song);
			} else if (
				currentSongAttributes.universe &&
				song.universe === currentSongAttributes.universe
			) {
	...
	const getSongLine = (
		{ title, artist, allTags, id, file }: SongNodeData,
		i: number,
	) => {
		return (
			<div
				...
				style={{
					...
					marginBottom: 4,
					background: i % 2 === 1 ? "#444" : "#666",
					padding: "0 3px 0 3px",
					boxSizing: "border-box",
					opacity: playedSongsIds.includes(id) ? 0.5 : 1,
				}}
			>
				{playedSongsIds.includes(id) && (
					<div style={{ alignSelf: "center" }}>
						<BsCheck color="lightgreen" />
					</div>
				)}
				<div>
					<div>{title}</div>
