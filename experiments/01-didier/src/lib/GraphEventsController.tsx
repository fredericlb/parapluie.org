...
const GraphEventsController: FC<{ setHoveredNode: (node: string | null) => void }> = ({ setHoveredNode }) => {
  ...
  const animateNode = (n: string, targetX: number, targetY: number, targetSize: number, back = false) => {
  ...
  useEffect(() => {
    registerEvents({
      ...
      doubleClickNode({node: debouncedHoveredNode}) {
        ...
        const nodesToKeep = graph.filterNodes(node =>
        ...
        const { x: centerX, y: centerY } = graph.getNodeAttributes(debouncedHoveredNode);
        ...
        sub.nodes().forEach((n, i) => {
          ...
          animateNode(n, (x / divider) + centerX, (y / divider) + centerY, origSize * 3);
        })
