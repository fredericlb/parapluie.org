...
const MessagesConsole: React.FC<{
	data: SongData[];
	...
	useEffect(() => {
		...
		invalidTransitions.forEach((it) => {
			msgs.push([
				"Le morceau cible de la transition n'existe pas :",
				it.source,
				"->",
				it.target,
			]);
		...
		if (orphans.length > 0) {
			msgs.push([
				`${orphans.length} morceaux sont orphelins -> `,
				{
					label: "Cliquez ici pour les lister",
					onClick: () => setSearch("@orphans"),
				},
			]);
		}
		...
		if (childless.length > 0) {
			msgs.push([
				`${childless.length} morceaux n'ont pas de descendance -> `,
				{
					label: "Cliquez ici pour les lister",
					onClick: () => setSearch("@childless"),
				},
			]);
		}
				...
				return (
					...
					<div key={i} style={{ color: "orange" }}>
						{xs.map((m) => {
							if (typeof m === "string") {
								return (
									<span key={m} style={{ marginRight: 4 }}>
										{m}
									</span>
								);
							}
