...
export function getGraph(
	dataset: SongData[],
	options: { showTransitions: boolean; tagsUniversesRatio: number },
) {
	const graph = new Graph({});
	const maxDuration = Math.max.apply(
		null,
		dataset.map((x) => x.duration),
	);
	...
	const universes = dataset.reduce<Record<string, number>>((prev, n) => {
		...
		if (prev[n.universe]) {
			prev[n.universe] = prev[n.universe] + 1;
		} else {
			prev[n.universe] = 1;
		}
		return prev;
	...
	dataset.forEach((d) => {
		const color = (
			d.bpm > 120
				? g2(Math.min((d.bpm - 120) / 60, 1))
				: g1(Math.max(0, d.bpm - 60) / 60)
		).toString({ format: "hex" });
		...
		if (r < 1) {
			...
			d.genres.forEach((genre) => {
				if (!graph.hasNode(`g_${genre}`))
					graph.addNode(`g_${genre}`, {
						type: "group",
						label: `g_${genre}`,
						color: "#880088",
						size: (1 - r) * 6,
					});
				graph.addEdge(d.file, `g_${genre}`, {
					weight: (1 - r) * 6,
					color: !options.showTransitions ? "#770077" : "#552255",
				});
			});
		}
		...
		if (r < 1) {
			if (!graph.hasNode(roundYear))
				graph.addNode(roundYear, {
					type: "group",
					label: roundYear,
					color: "#008888",
					size: (1 - r) * 3,
				});
			graph.addEdge(d.file, roundYear, {
				weight: (1 - r) * 3,
				color: !options.showTransitions ? "#005555" : "#003333",
			});

			if (!graph.hasNode(d.energylevel))
				graph.addNode(d.energylevel, {
					type: "group",
					label: `Énergie : ${d.energylevel}`,
					color: "#888800",
					size: (1 - r) * 5,
				});
			graph.addEdge(d.file, d.energylevel, {
				weight: (1 - r) * 5,
				color: !options.showTransitions ? "#555500" : "#333300",
			});
		}
...
export function filterOrphans<T extends SongData>(data: T[]): T[] {
	const transitionsToFrom = data.reduce(
		(acc, curr) => {
			// biome-ignore lint/complexity/noForEach: <explanation>
			(acc.transitions ?? []).forEach((tr) => {
				...
				acc[id].push(curr.file);
			});
			acc[curr.file] = (acc.transitions ?? []).map((x) => `${x}.mp3`);
			return acc;
		},
		{} as Record<string, string[]>,
	);

	return Object.entries(transitionsToFrom)
		.filter(([, target]) => target.length === 0)
		.map((x) => data.find((d) => d.file === x[0]) as T);
}

export function filterChildless<T extends SongData>(data: T[]): T[] {
	const transitionsFromTo = data.reduce(
		(acc, curr) => {
			acc[curr.file] = (curr.transitions ?? []).map((x) => `${x}.mp3`);
			return acc;
		},
		{} as Record<string, string[]>,
	);

	return Object.entries(transitionsFromTo)
		.filter(([, target]) => target.length === 0)
		.map((x) => data.find((d) => d.file === x[0]) as T);
}
...
export function filterSearch(search: string, graph: Graph): SongNodeData[] {
	...
	const nodes = Array.from(graph.nodeEntries())
		.map((x) => x.attributes)
		.filter((x) => x.type === "song" && !x.hidden) as SongNodeData[];
